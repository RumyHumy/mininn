...
// TODO: Abstract the element access in matricies
// TODO: Reimplement matricies in row-order fashion
...
void MSEDeriv(Mat2d *exp, Mat2d *act, Mat2d *res)
{
	double **expd=exp->dat;
	double **actd=act->dat;
	double **resd=res->dat;
	...
	MATFOR_ROWS(exp,r)
		resd[r][0] = 2.0*(actd[r][0]-expd[r][0])/exp->rows;
}
...
void NetLayerForw(NetLayer *layer, Mat2d *in, Mat2d *out)
{
	double **outd = out->dat;
	double **ind  = in->dat;
		...
		double lrate)
{
	...
	// dE/dX = dE/dY mul f'(X)
	MATFOR2D(gradIn,r,c)
		gradInd[r][c] *= layer->activ.funcDeriv(aind[r][c]);
	...
	NetActiv activs[])
{
	...
	*net = (Net){ .lcount=lcount, .layers=NULL };
	...
	net->layers = malloc(lcount*sizeof(*net->layers));
	if (!net->layers)
	{
		free(net);
		return NULL;
	...
	size_t epochs)
{
	...
	//	Mat2d *eOut=tOut[i];
	//	for (int l=0; l<net->lcount; l++){
	//		NetLayerForw(net->layers[l],in,out);
	//	}
	//	Mat2d gradIn = Mat2dAlloc(eOut);
	//	MSEDeriv(eOut,out,gradIn);
	//}
	//Mat2dFree(out);
}
...
char UTNetLayer()
{
	...
	NetLayer *layer = NetLayerAlloc(2, 3, ACTIV_SIGMOID);
	if (!layer) return 0;
...
char UTNetLayerBackw()
{
	...
	if (!layer || !in || !out || !eout || !grad || !gradOut)
		goto cleanup;
	...
	double **outd = out->dat;
	flag =
		   FCOMP(out->dat[0][0], eout->dat[0][0], 3)
		&& FCOMP(out->dat[1][0], eout->dat[1][0], 3)
		&& FCOMP(out->dat[2][0], eout->dat[2][0], 3);
